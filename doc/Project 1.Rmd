...
For topic modeling, I prepare a corpus of sentence snipets as follows: I remove punctuations, lowercase, common(nonsense) words, duplicate key words and blankspaces. 

#### 3.3 Data Analysis: Sentiment Analysis
Sentiment analysis can help us determine whether the sentence is positive, negative or neutral. It is useful for us to gain an overview of all schools' opinions behind certain topics. And we can see how philosophers shift between different sentiments.
...
First two barcharts show that school Feminism spent lots of time exploring women social positions. This is what we expect, but besides Feminism,  Aristotle tends to have more texts related to women. In additionm, in year 1949, female is a popular topic.  
...
## 4.1 Topic Modeling 
We first perfromed text mining and text basic processing, then I gengerate document-term matrices and run LDA. I set to select 15 topic groups, and it is shown in the following:
...
From above wordcloud, we can see that keywords such as like, must, eggs, first are frequently used by school Aristotle when they talked about women. The top twenty keywords include animals, organ, seman, distinguished and so on. Below is an example of Aristotle's sentence with keyword organ:
...
From this example, we can see that for Aristotle, women's purpose is to give birth to children. That opinion is biased and not correct. 
...
From sentiment bar chart, we could see that besides two highest bars of negative and positive, most school Aristotle's sentences contain words which have sentiments such as fear, sadness and trust.



# 6. Conclusion
All in all, based on the analysis, we can see that There has been increased empirical research on women’s underrepresentation in philosophy at various levels; however, there is a clear pattern that women are underrepresented in philosophy school Aristotle. At year 1949, Feminist philosophy has become mainstream, and school Feminism bring attention to women: What are women's role" and discussed gender of roles and responsibilities. 



# 7. Issues and Potential Biases
There are issues on sentiment analysis. Since there are anbiguous words used to describe authors' perspectives on gender roles; for example: "Conversely, if men in general are taller than women, then the tallest man is taller than the tallest woman." It is clearly that Aristotle thought women are not equal to men and female are inferior; however, words he used are taller, tallest and than, so sentiment analysis thinks of this sentence with emotions trust, positive. That is totally opposite of Arritotle's view, so sentiment analysis has some limitations on philosophy text data, and for further study, we might need to manually go through each text and label them. 
